...
typedef JsonRpcWebsocketClientOnError = void Function(Object error, [StackTrace? stackTrace]);
...
class JsonRpcWebsocketClient<R> extends Client<R> {
  ...
  Duration? _remaining(final DateTime start, { required final Duration? timeLimit, }) {
  ...
  FutureOr<Duration> _backoff(
    ...
    final Duration? limit = timeLimit ?? this.timeLimit;
    final int ms = i < schedule.length ? schedule[i] : (schedule.isEmpty ? 0 : schedule.last);
  ...
  Future<void> _connect({ 
    ...
    for (int i = 0; i < maxAttempts && !_disposed; ++i) {
      try {
        ...
        final Duration delay = await _backoff(i, schedule: backoffSchedule, timeLimit: remaining);
        final WebSocketChannel client = WebSocketChannel.connect(uri, protocols: protocols);
        _subscribe(client.stream.cast<R>());
        await timeout(client.ready, remaining != null ? remaining - delay : null);
        _client = client;
        onConnect?.call();
        return;
  ...
  int? _responseId(final dynamic response) {
    ...
    return response is List ? (response.isEmpty ? null : response.first[key]) : response[key];
  }
  ...
  void _onData(final R data) async {
    ...
    if (isPing(data)) {
      ...
      final dynamic decoded = await decoder.convert(data); // Map or List<Map> or Empty List (PING).
  ...
  void _cancel(final int? id, final Object error, [final StackTrace? stackTrace]) {
    final Completer? pending = _pending.remove(id);
  ...
  Future<T> _handler<T>(
    ...
    try {
      ...
      if (id == null) {
        throw const JsonRpcException('Websocket requests must define a unique `id`.');
      }
