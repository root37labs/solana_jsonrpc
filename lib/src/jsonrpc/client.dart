...
abstract class Client<R> {
  ...
  FutureOr<T> onTimeout<T>() => Future.error(TimeoutException('The request has timed out.'));
  ...
  Future<T> timeout<T>(final Future<T> request, final Duration? timeLimit) {
    ...
    return limit != null ? request.timeout(limit, onTimeout: onTimeout) : request;
  }
  ...
  Future<JsonRpcSuccessResponse<T>> send<T>(
    ...
    final JsonRpcResponseDecoder<Map<String, dynamic>, JsonRpcResponse<T>> decode, {
    final JsonRpcClientConfig? config,
  }) async {
    final List<int> body = await encoder.convert(request);
    final Map<String, dynamic> json = await handler(body, config: config, id: request.id);
